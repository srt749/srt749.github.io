...
ALOE.recursiveParse = function (param) {
    try {
        ...
        //non bool/num strings error immediately so return
        return param;
    }
};
...
DataManager.extractSaveContents = function (contents) {
    ...
    if (contents.gameNotepad) {
        $gameNotepad = contents.gameNotepad;
    }
};
...
Game_Notepad.prototype.allNotes = function () {
    return this._data.filter(function (a) {
        return a;
    });
};
...
Game_Notepad.prototype.keyExists = function (key) {
    return this.allNotes().map(function (a) {
        return a.key;
    }).indexOf(key) >= 0;
};

Game_Notepad.prototype.getIdByKey = function (key) {
    var index = this.allNotes().map(function (a) {
        return a.key;
    }).indexOf(key);
    return this.note(index).id;
};

Game_Notepad.prototype.addNote = function (title, details, key, titleLocked, detailsLocked, deleteLocked, hidden) {
    title = title || [""];
    details = details || [""];
    key = key || "";
...
Game_Notepad.prototype.modifyNoteByKey = function (key, title, details, titleLocked, detailsLocked, deleteLocked, hidden) {
    var index = this.allNotes().map(function (a) {
        return a.key;
    }).indexOf(key);
    this._data[index].title = title || this.titleById(id);
...
Game_Notepad.prototype.appendNoteByKey = function (key, title, details) {
    var index = this.allNotes().map(function (note) {
        return note.key;
    }).indexOf(key);
    if (title) {
        this._data[index].title[0] += title;
    }
    if (details) {
        this._data[index].details = this._data[index].details.concat(details);
    }
};
...
Game_Notepad.prototype.isNoteTitleModifiable = function (id) {
    ...
    if (index >= 0) {
        return !this.note(index).titleLocked;
    }
};
...
Game_Notepad.prototype.isNoteDetailsModifiable = function (id) {
    ...
    if (index >= 0) {
        return !this.note(index).detailsLocked;
    }
};
...
Scene_Notepad.prototype.commandSaveNote = function () {
    ...
    var index = $gameNotepad.allNotes().map(function (a) {
        return a.id;
    }).indexOf(id);
    if (index < 0) {
...
Scene_Notepad.prototype.processRight = function () {
    if (this._activeWindow.checkRight()) {
        this._activeWindow._index++;
    } else if (this._activeWindow._index === this._activeWindow.text[this._activeWindow._row].length && this._activeWindow._row < this._activeWindow.text.length - 1) {
        this._activeWindow._row++;
...
Window_NotepadCommand.prototype.removeCommand = function (symbol) {
    var index = this._list.map(function (a) {
        return a.symbol;
    }).indexOf(symbol);
...
Window_NotepadBase.prototype.checkRight = function () {
    return (this._maxLength === 0 || this._index < this._maxLength) && this._index < this.text[this._row].length;
